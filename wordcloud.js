...
document.getElementById("getURL").oninput = async function() {
    let userURL = this.value;
    console.log(userURL);

    try {
        let isURL = userURL.startsWith("http") || userURL.startsWith("www");
    
        if (!isURL) {
            throw "this is not a URL!"
        }
        else if (isURL) {
            let url = new URL(userURL);
            let title = url.pathname.split("/").pop();

            let apiUrl = `https://en.wikipedia.org/w/api.php?origin=*&action=query&prop=extracts&format=json&explaintext=&titles=${title}`;
            ...
            if (response.ok) { 
                ...
                let sourceText = pages[0].extract;

                let sourceDoc = document.getElementById("wc_webpage");
                sourceDoc.textContent = sourceText;
